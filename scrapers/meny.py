"""Skraper for Meny — meieri/melk.

Strategi: Kaller NorgesGruppens produkt-API direkte. Meny, Spar, Joker og Kiwi
deler samme API — bare kjede-ID (1300 = Meny) og GLN skiller.

Endepunktet er samme som nettsiden selv bruker når du surfer meny.no.
"""
...
API_URL = "https://platform-rest-prod.ngdata.no/api/products/1300/7080001150488"
CATEGORY_FACET = "Categories:Meieri & egg;ShoppingListGroups:Melk"
PAGE_SIZE = 48
...
def parse_product(src: dict) -> dict:
    ...
    # subtitle inneholder ofte merkenavnet bakerst eller fremst — fjern det,
    # ellers ender vi opp med "... Tine Tine" eller lignende i display-navnet.
    sub = subtitle
    if brand:
    ...
    # Standardiser størrelse til liter/kg der det gir mening.
    ...
    url = f"https://meny.no/varer{slug}" if slug else None
...
def main() -> None:
    conn = connect()
    store_id = get_store_id(conn, "meny")

    raw_items = fetch_products()
    print(f"Fant {len(raw_items)} produkter fra Meny (melk).")
